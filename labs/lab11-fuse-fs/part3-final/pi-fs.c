...
static int pi_open(const char *path, struct fuse_file_info *fi) {
    note("opening\n");
    int retv;
    dirent_t *dir = file_lookup(&retv, root, path, perm_rd); // See .h for perm_read
    return retv;
}
...
static int pi_read(const char *path, char *buf, size_t size, 
    ...
    if (offset > e->f->n_data)
        return 0;
    size = XMIN(size, e->f->n_data - offset); // read size bytes or what's left
...
static int pi_write(const char *path, const char *buf, size_t size, 
    ...
    if(!e)
        return retv;
    if (e->on_wr) e->on_wr(e, path, buf, size, offset, fi); // Goto write
    else {
        // Copy size bytes from buffer to the file structure's data
        if (offset + size > e->f->n_alloc) file_realloc(e->f, offset + size);
...
static int pi_truncate(const char *path, off_t length) {
    ...
    // XXX: should zero fill a truncated region
    if (length > e->f->n_alloc) {
        file_realloc(e->f, length);
    }
    e->f->n_data = length;
    return 0;
}
...
int ingest_fd = -1;
int supply_fd = -1;
const char *kShell = "my-install -exec ./redirect ../../lab10-shell/shell-unix-side/pi-shell ../../lab10-shell/shell-pi-side/pi-shell.bin";
